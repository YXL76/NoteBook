...
- 0-1 分布
  - $X = X(e) = \left\{ \begin{array}{ll} 0, & e=e_1 \\ 1, &e=e_2 \end{array} \right.$
- 二项分布
  - $X\sim b(n,p)$
  - $P(X=k)=\left(\begin{array}{ll} n \\ k \end{array}\right)p^k(1-p)^{n-k}$
  - $n=1$是为 0-1 分布
- 泊松分布
  - $X\sim\pi(\lambda)$
...
- 均匀分布
  - $X\sim U(a,b)$
  - $f(x) = \left\{ \begin{array}{ll} \frac{1}{b-a}, & a<x<b \\ 0, & other \end{array} \right.$
  - $F(x) = \left\{ \begin{array}{ll} 0, & x<a \\ \frac{x-a}{b-a}, & a\leq x<b \\ 1, &x\geq b \end{array} \right.$
- 指数分布
  - $f(x) = \left\{ \begin{array}{ll} \frac{1}{\theta}e^{-x/\theta}, & x>0 \\ 0, & other \end{array} \right.$
  - $F(x) = \left\{ \begin{array}{ll} 1-e^{-x/\theta}, & x>0 \\ 0, & other \end{array} \right.$
- 正态分布
  - $X\sim N(\mu,\sigma^2)$
  - $\begin{aligned} f(x)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}},\ -\infty<x<\infty \end{aligned}$
...
- $Z=X+Y$的分布
  - $\begin{aligned} f_{X+Y}(z) & =\int_{-\infty}^\infty f(z-y,y)\text{d}y \\ & =\int_{-\infty}^\infty f(x,z-x)\text{d}y \end{aligned}$
  - 相互独立：$\begin{aligned} f_{X+Y}(z) & =\int_{-\infty}^\infty f_X(z-y)f_Y(y)\text{d}y \\ & =\int_{-\infty}^\infty f_X(x)f_Y(z-x)\text{d}y \end{aligned}$
- $Z=\frac{Y}{X},\ Z=XY$的分布
...
- 伯努利大数定理：$f_A$是进行$n$次独立实验中事件A发生的次数，$p$是A在每次实验发生的概率，则对于任意正数$\varepsilon>0$，有$E(X_k)=\mu$。有$\begin{aligned} \lim_{n\to\infty}P \left\{ \left| \frac{f_A}{n}-p \right|<\varepsilon \right\}=1 \end{aligned}$
